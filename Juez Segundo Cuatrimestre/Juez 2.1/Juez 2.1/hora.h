...
public:
	...
	int horaSalida(std::vector<Hora> &h1) 
	{
		int i = 0;
		...
		if (i >= h1.size()) throw std::out_of_range("NO");
		return i;
	}
...
inline std::istream & operator >> (std::istream & in, Hora & hora) { //istream para que pueda ser diferentes a telefonos
	int horas, minutos, segundos;
	char puntos;
